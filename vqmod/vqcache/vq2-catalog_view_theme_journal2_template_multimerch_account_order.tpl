	...
	<div class="row"><?php echo $column_left; ?><?php echo $column_right; ?>
		...
		<div id="content" class="<?php echo $class; ?>"><?php echo $content_top; ?>
                                ...
                                <div class="row">
                                    ...
                                    <div class="order-actions xl-20 xs-100">
										<?php if($this->config->get('msconf_reviews_enable') && !$product['review']) :?>
											<a href ="<?php echo $this->url->link('customer/review/create', 'product_id=' . $product['product_id'] . '&order_id=' . $order['order_id'] . '&order_product_id=' . $product['order_product_id']) ;?>" class="action btn button btn-primary"><?php echo $ms_order_feedback ;?></a>
										<?php endif ;?>
                                        <a href ="<?php echo $this->url->link('product/product', 'product_id=' . $product['product_id']) ;?>" class="action btn button btn-default pull-right"><?php echo $ms_order_buy_again ;?></a>
                                        <a href="<?php echo $this->url->link('account/return/add', 'order_id=' . $order['order_id'] . '&product_id=' . $product['product_id'], true) ?>" class="action btn button btn-link pull-right"><?php echo $ms_order_return ;?></a>
                                    </div>
                                </div>
                            </div>
						<?php endforeach ;?>
			...
			<?php if (empty($orders['orders'])) { ?>
				...
				<div class="buttons">
					<div class="pull-right"><a href="<?php echo $this->url->link('account/account', '', 'SSL'); ?>" class="btn button btn-primary button"><?php echo $button_continue; ?></a></div>
				</div>
			<?php } ?>
		</div>
