	...
	<div class="tabs-content">
		<?php foreach ($seller_histories as $history) { ?>
			<?php if(!empty($history['entries']) || $this->config->get('mmess_conf_enable')) { ?>
				<div class="tab-pane tab-content <?php if($history['suborder_id'] == $active_tab) { ?>active<?php } ?>" id="seller_tab<?php echo $history['suborder_id']; ?>">
						...
						<div class="ms-account-conversation">
							<div class="ms-messages">
								<?php foreach ($history['order_messages'] as $message) { ?>
									<div class="
				ms-message
			 <?php echo $message['sender_type_id'] == MsConversation::SENDER_TYPE_ADMIN ? 'admin' : ($message['sender_type_id'] == MsConversation::SENDER_TYPE_SELLER ? 'seller' : ''); ?>">
								...
								<form id="ms-message-form<?php echo $history['suborder_id']; ?>" class="ms-form form-horizontal">
									...
									<div class="col-sm-2">
										<div class="buttons text-center">
											<button type="button" class="btn button btn-default ms-message-upload"><i class="fa fa-upload"></i> <?php echo $button_upload; ?></button>
											<button data-suborder_id="<?php echo $history['suborder_id']; ?>" type="button" class="btn button btn-primary ms-order-message"><?php echo $ms_post_message; ?></button>
										</div>
									</div>
								</form>
							</div>
